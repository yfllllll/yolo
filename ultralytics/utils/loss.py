...
from ultralytics.utils.tal import RotatedTaskAlignedAssigner, TaskAlignedAssigner, dist2bbox, dist2rbox, make_anchors, wasserstein_loss, gcd_loss
...
class BboxLoss(nn.Module):
    ...
    def __init__(self, reg_max=16):
        ...
        # GCD
        self.gcd_loss = True
        self.iou_ratio = 0.2 # total_iou_loss = self.iou_ratio * iou_loss + (1 - self.iou_ratio) * (nwd_loss or gcd_loss)
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask, mpdiou_hw=None):
        ...
        if self.nwd_loss:
            ...
            # 现在不需要乘以0.0002了，之前是因为矩阵广播尺寸错误的问题导致的
            gcd_l = ((1.0 - gcd) * weight.squeeze()).sum() / target_scores_sum
