...
# 仅保留核心统计量+EMA相关参数（无需手动控制）
_GCD_STATS = {
    'E_Dgc2': None,    # 全局均值（EMA更新）
    'Med_Dgc2': None,  # 全局中值（滑动更新）
    'count': 0         # 累计样本数（用于EMA权重）
}

def gcd_loss(pred, target, eps=1e-7, mode='exp', gamma=1):
    """
    极简版BA-AW-GCD：指数滑动平均统计全局分布，无额外滑动参数
    Args:
        pred: 预测框，shape=[B, N, 4] 或 [N, 4]，格式(x1,y1,x2,y2)
        target: 真实框，shape同pred
        eps: 避免除零
        mode: 损失映射模式
        gamma: norm_sqrt模式超参数
    Returns:
        gcd: 改进后的GCD值
    """
    global _GCD_STATS
    ...
    # -------------------------- 2. 修正视角拆分 --------------------------
    center_distance_p = (whs[..., 0] / (w1 + eps)) ** 2 + (whs[..., 1] / (h1 + eps)) ** 2
    wh_distance_p = (((w1 - w2) / (w1 + eps)) ** 2 + ((h1 - h2) / (h1 + eps)) ** 2) / 4
    S_p = center_distance_p + wh_distance_p
    
    center_distance_t = (whs[..., 0] / (w2 + eps)) ** 2 + (whs[..., 1] / (h2 + eps)) ** 2
    wh_distance_t = (((w1 - w2) / (w2 + eps)) ** 2 + ((h1 - h2) / (h2 + eps)) ** 2) / 4
    S_t = center_distance_t + wh_distance_t
    
    # -------------------------- 3. 原始GCD距离平方（逐框） --------------------------
    gcd_2_original = (S_p + S_t) / 2
    
    # -------------------------- 4. 指数滑动平均更新全局统计量（核心简化） --------------------------
    flat_gcd2 = gcd_2_original.detach().reshape(-1)  # 当前batch的所有框D_gc²
    batch_size = len(flat_gcd2)
    batch_mean = torch.mean(flat_gcd2).item()
    batch_median = torch.median(flat_gcd2).item()
    
    # 初始化：首次调用直接赋值
    if _GCD_STATS['E_Dgc2'] is None or _GCD_STATS['Med_Dgc2'] is None:
        _GCD_STATS['E_Dgc2'] = batch_mean
        _GCD_STATS['Med_Dgc2'] = batch_median
        _GCD_STATS['count'] = batch_size
    else:
        # 指数滑动平均更新均值（EMA）：权重随累计样本数动态调整
        total_count = _GCD_STATS['count'] + batch_size
        ema_weight = batch_size / total_count  # 新样本权重=当前batch数/累计总数
        _GCD_STATS['E_Dgc2'] = (1 - ema_weight) * _GCD_STATS['E_Dgc2'] + ema_weight * batch_mean
        
        # 滑动更新中值（简化版：加权平均，兼顾历史和当前）
        _GCD_STATS['Med_Dgc2'] = 0.9 * _GCD_STATS['Med_Dgc2'] + 0.1 * batch_median
        _GCD_STATS['count'] = total_count
    
    # -------------------------- 5. 逐框计算α --------------------------
    k = 1 / (_GCD_STATS['E_Dgc2'] + eps)
    tau = _GCD_STATS['Med_Dgc2']
    alpha = torch.sigmoid(k * (tau - gcd_2_original))  # 逐框α
    
    # -------------------------- 6. 加权后的GCD --------------------------
    gcd_2 = (alpha * S_p + (1 - alpha) * S_t) / 2
    
    # -------------------------- 7. 损失映射（保留原逻辑） --------------------------
    if mode == 'exp':
        gcd = torch.exp(-torch.sqrt(gcd_2 + eps))
    elif mode == 'sqrt':
        gcd = torch.sqrt(gcd_2 + eps)
    elif mode == 'log':
        gcd = torch.log(gcd_2 + 1 + eps)
    elif mode == 'norm_sqrt':
        gcd = 1 - 1 / (gamma + torch.sqrt(gcd_2 + eps))
    elif mode == 'w2':
        gcd = gcd_2
    else:
...
def reset_gcd_stats():
    global _GCD_STATS
    _GCD_STATS = {'E_Dgc2': None, 'Med_Dgc2': None, 'count': 0}
