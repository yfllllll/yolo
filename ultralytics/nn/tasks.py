...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    if scales:
        ...
        if len(scales[scale]) == 3:
            ...
            raise ValueError(f"Unexpected scales length: {len(scales[scale])}")
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):
        ...
        # ----- 新增：node_mode 动态模块名替换 -----
        if m == 'node_mode':
            m = d[m]                              # 从顶层字典取真实模块名
            if len(args) > 0 and args[0] == 'head_channel':
                args[0] = int(d[args[0]])         # 替换 head_channel 占位符
        ...
        if m in {
            Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv,
            Focus, BottleneckCSP, C1, C2, C2f, RepNCSPELAN4, ELAN1, ADown, AConv, SPPELAN, C2fAttn,
            C3, C3TR, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, RepC3, PSA, SCDown, C2fCIB,
            C2f_Star,                              # <-- 新增
        }:
            if args[0] == 'head_channel':         # 支持 head_channel
                args[0] = d[args[0]]
            c1, c2 = ch[f], args[0]
            if c2 != nc:
            ...
            args = [c1, c2, *args[1:]]
            if m in {BottleneckCSP, C1, C2, C2f, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3, C2fCIB, C2f_Star}:
                args.insert(2, n)
                n = 1
        ...
        # ----- 新增：Fusion (支持 fusion_mode) -----
        elif m is Fusion:
            args[0] = d[args[0]]
            c1 = [ch[x] for x in f]
            c2 = sum(c1) if args[0] == 'concat' else ch[f[0]]
            args = [c1, args[0]]

        # ----- 新增：ContextGuidedBlock_Down -----
        ...
        elif m in {Detect, Detect_Efficient, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, v10Detect}:
